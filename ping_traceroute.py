...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        icmp_errors = {                 # ICMP error dict (type 3 and 11)
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            # compare each value and identify if this is a valid response.
            
            # get expected
            expected_ID = self.getPacketIdentifier()
            expected_SN = self.getPacketSequenceNumber()
            expected_D = self.getDataRaw()

            # get received
            received_ID = icmpReplyPacket.getIcmpIdentifier()
            received_SN = icmpReplyPacket.getIcmpSequenceNumber()
            received_D = icmpReplyPacket.getIcmpData()

            # check against each other
            identifier_isValid = expected_ID == received_ID
            sequenceNumber_isValid = expected_SN == received_SN
            data_isValid = expected_D == received_D

            # set to result
            icmpReplyPacket.setIcmpIdentifier_isValid(identifier_isValid)
            icmpReplyPacket.setIcmpSequenceNumber_isValid(sequenceNumber_isValid)
            icmpReplyPacket.setIcmpData_isValid(data_isValid)

            # print if error found
            if not identifier_isValid: print(f"ERROR FOUND:\n\tExpected Identifier: {expected_ID}\n\tReceived Identifier: {received_ID}\n\tValid: {identifier_isValid}")
            if not sequenceNumber_isValid: print(f"ERROR FOUND:\n\tExpected Sequence Number: {expected_SN}\n\tReceived Sequence Number: {received_SN}\n\tValid: {sequenceNumber_isValid}")
            if not data_isValid: print(f"ERROR FOUND:\n\tExpected Data: {expected_D}\n\tReceived Data: {received_D}\n\tValid: {data_isValid}")

            # set icmpReplyPacket validity
            icmpReplyPacket.setIsValidResponse(identifier_isValid and sequenceNumber_isValid and data_isValid)
        ...
        def sendEchoRequest(self, tracing): # (tracing) flag check if traceroute or ping
            ...
            try:
                ...
                if timeLeft <= 0:
                    ...
                    # received ICMP error 
                    if (icmpType, icmpCode) in self.icmp_errors:
                        # return relevant info [rtt, dest address, icmpType, icmpCode, icmp Error msg]
                        return [round((timeReceived - pingStartTime) * 1000), addr[0], icmpType, icmpCode, self.icmp_errors[(icmpType, icmpCode)]]
                    else:
                        # Normal processing of echo reply
                        icmpReplyPacket = IcmpHelperLibrary.IcmpPacket_EchoReply(recvPacket)
                        # prints errors if found
                        self.__validateIcmpReplyPacketWithOriginalPingData(icmpReplyPacket)
                        if icmpReplyPacket.isValidResponse():
                            # print packet info if "pinging"
                            if not tracing: icmpReplyPacket.printResultToConsole(self.getTtl(), timeReceived, addr)
                        else:
                            print(f"Invalid ICMP packet received from {addr[0]}")
                        
                        # get RTT for packet to return
                        rtt = round((timeReceived - icmpReplyPacket.getTimeSent()) * 1000)
                        # return relevant info [rtt, dest address, icmpType, icmpCode, icmp Error msg]
                        return [rtt, addr[0], icmpType, icmpCode, "Reached Destination"]
            except timeout:
    ...
    class IcmpPacket_EchoReply:
        ...
        # return time (seconds) packet was sent                                          
        def getTimeSent(self):
            bytes = struct.calcsize("d")
            return struct.unpack("d", self.__recvPacket[28:28 + bytes])[0]
    ...
    def __sendIcmpEchoRequest(self, host):
        ...
        # hold RTTs for received packets, for later printing of "Echo" results
        RTTs = []
        ...
        for i in range(numSent):
            ...
            # ping "host"
            recv = icmpPacket.sendEchoRequest(False)
            
            # returns info array if packet received, None if timeout
            if recv != None:
                if recv[2] == 0:
                    RTTs.append(recv[0]) # get rtt

            icmpPacket.printIcmpPacketHeader_hex() if self.__DEBUG_IcmpHelperLibrary else 0
            icmpPacket.printIcmpPacket_hex() if self.__DEBUG_IcmpHelperLibrary else 0
            # we should be confirming values are correct, such as identifier and sequence number and data
        
        # received packets, print "Echo" info
        if RTTs:
            print(f"Ping statistics for '{host}'\n\tPackets: Sent = {numSent}, Lost = {numSent - len(RTTs)} ({round((numSent - len(RTTs)) / numSent * 100, 2)}% loss)")
            print(f"Approximate round trip time in milli-seconds:\n\t Minimum = {min(RTTs)}ms, Maximum = {max(RTTs)}ms, Average = {round(sum(RTTs)/len(RTTs))}ms\n")
        else:
            print(f"Ping statistics for '{host}'\n\tPackets: Sent = {numSent}, Lost = {numSent} (100% loss)")
            print(f"Approximate round trip time in milli-seconds:\n\t Minimum = X, Maximum = X, Average = X\n")
    ...
    def __sendIcmpTraceRoute(self, host):
        ...
        for hops in range(1, maxHops+1):
            icmpPacket = IcmpHelperLibrary.IcmpPacket()
            randomIdentifier = (os.getpid() & 0xffff)
            packetIdentifier = randomIdentifier
            packetSequenceNumber = hops

            icmpPacket.buildPacket_echoRequest(packetIdentifier, packetSequenceNumber)
            icmpPacket.setIcmpTarget(host)
            
            # set "lifetime"
            icmpPacket.setTtl(hops)
            
            recv = icmpPacket.sendEchoRequest(True)
            
            icmpPacket.printIcmpPacketHeader_hex() if self.__DEBUG_IcmpHelperLibrary else 0
            icmpPacket.printIcmpPacket_hex() if self.__DEBUG_IcmpHelperLibrary else 0
            
            # if packet received, print relevant info
            if recv:
                print(f'TTL={hops}\tRTT={recv[0]}ms    Type={recv[2]}\tCode={recv[3]}\t({recv[4]})    {recv[1]}')
                # if ICMP error type 3 or 0, we've reached "host"
                if recv[2] == 3 or recv[2] == 0:
                    print('Trace Complete.\n')
                    return

        # exceeded "maxHops"
        print('Trace Ended - "maxHops" Reached.')
    ...
    def sendPing(self, targetHost):
        ...
        if not self.__DEBUG_IcmpHelperLibrary: print("Starting Ping to (" + targetHost + ")")
        self.__sendIcmpEchoRequest(targetHost)

    def traceRoute(self, targetHost):
        print("traceRoute Started...") if self.__DEBUG_IcmpHelperLibrary else 0
        
        if not self.__DEBUG_IcmpHelperLibrary: print("Traceroute to (" + targetHost + ")")
...
def main():
    ...
    icmpHelperPing.traceRoute("1.0.0.1") # Australia
    icmpHelperPing.traceRoute("1.1.1.1") # Asia 
